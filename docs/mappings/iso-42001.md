...
| **Documentation and traceability** (Clause 7.5 + Annex A) | Partial | See the correction note below. `AuditRecord` v4 (`contracts/v4/audit_record.py`) **specifies** Ed25519 signing and `previous_hash` → `current_hash` chaining; the signed envelope chain that is actually written lives in `phionyx-mcp-server`, at the tool boundary. Canonical block 44 `audit_layer` computes an integrity score and does neither. |
...
**Score:** 0 Full · 9 Partial · 6 Gap.

> ### Correction — 2026-08-02
>
> Until this date the *Documentation and traceability* row read **Full (within
> runtime perimeter)** and cited "`audit_layer` (block 44) — Ed25519-signed hash
> chain; `governed_response` envelope". Both halves of that citation were
> checked on 2026-08-02 and neither resolves to what it claims:
>
> - `phionyx_core/pipeline/blocks/audit_layer.py` contains **no** occurrence of
>   Ed25519, signing, hash chaining or `AuditRecord`. When no processor is
>   injected it computes a heuristic integrity score from response length and
>   the presence of a physics state, and returns it as block data. It writes no
>   record.
> - `governed_response` appears in the codebase as an OpenTelemetry semantic
>   attribute name, not as the JSON-Schema envelope the row described.
>
> The Ed25519 hash chain is real, and it is elsewhere: `AuditRecord` v4 specifies
> it, and `phionyx-mcp-server` writes signed, hash-chained envelopes at the MCP
> tool boundary. Neither is canonical block 44.
>
> The rating is corrected to **Partial** and the citation now names components
> that exist. This is the row the Measurement Axioms self-audit
> (2026-08-01) recorded as *"a compliance mapping rating a control Full on
> citations that resolve to nothing"* — the same class of defect the doctrine
> exists to name, found in our own public compliance artefact.
>
> A reader who relied on the previous row for an AIMS evidence claim should
> re-read it. Nothing in that row was independently verified before publication;
> the verification that produced this correction is `grep` against the named
> files, which anyone can repeat.

No row is Full. The strongest row is **documentation and traceability** within the runtime perimeter — the AuditRecord v4 *contract* is structurally aligned with the kind of per-decision audit trail Annex A controls expect, while canonical runtime production is under alignment (see the correction note). Every Partial row identifies the organisational deliverable that complements Phionyx's contribution. The 6 Gap rows are **management-system controls** that no software library can satisfy.

> **Why deferred from earlier roadmap:** Master plan v2.0 deferred this mapping (P2-deferred) because OWASP + EU AI Act + NIST already covered the runtime-side evidence surface; ISO/IEC 42001 *organisational* coverage is heavily Gap-leaning by Phionyx's architecture. The mapping is now published as a **draft** because (a) the organisation plans to pursue formal AIMS alignment, (b) the standards-reading note for 42001 was completed in the internal development history, making this a logical sequel, and (c) explicit acknowledgement of organisational gaps is itself a credible signal.
...
## Documentation and traceability (**Partial** — within runtime perimeter)
...
- The signed, append-only envelope chain actually written today lives in `phionyx-mcp-server`, at the MCP tool boundary. Canonical block 44 `audit_layer` computes an integrity score and writes no record (see the correction note above).
...
**Coverage.** Partial — see the correction note of 2026-08-02 above: the traceability *contract* is specified and the signed chain is written at the MCP boundary; canonical Core production is under alignment.
...
- Standards reading notes — internal development history
- In-repo architecture documentation (README, `ARCHITECTURE.md`, `phionyx_core/contracts/`)
- Evidence Matrix — <https://phionyx.ai/evidence>

---

> **Status:** v0.1 draft. Will graduate to non-draft after either (a) formal AIMS certification enables Annex A identifier verification, or (b) a deployer's AIMS audit produces concrete feedback on which mapping rows actually carry weight in practice. Until then, treat as a structural draft — directionally correct, identifier-level uncertain.
