    ...
    async def execute(self, context: BlockContext) -> BlockResult:
        try:
            ...
            _outcome = BlockOutcome(
                ...
                measurement=errored(
                    "goal evaluation raised; goals_evaluated 0 means it crashed, not that there were none",
                    inputs_present=True,
            ...
            return BlockResult(
                block_id=self.block_id,
                status="ok",
                data={**({"goals_evaluated": 0, "error": str(e)}), "block_outcome": _outcome.to_record_fields()},
            )
